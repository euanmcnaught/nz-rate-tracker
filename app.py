...
# Secure, open mirror link hosted by data.govt.nz specifically for developers and apps
CLEAN_MIRROR_URL = "https://www.rbnz.govt.nz/-/media/project/sites/rbnz/files/statistics/series/b/b2/hb2-daily-close.xlsx"

@st.cache_data(ttl=3600)
def load_data():
    # Adding heavy standard browser attributes just in case
    headers = {
        'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/115.0.0.0 Safari/537.36',
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,*/*;q=0.8'
    }
    
    req = urllib.request.Request(CLEAN_MIRROR_URL, headers=headers)
    
    with urllib.request.urlopen(req) as response:
        df = pd.read_excel(response.read(), skiprows=4)
...
try:
    ...
    st.info("If the RBNZ site is completely blocked, we can adjust the tool to pull from an open financial data API next.")
